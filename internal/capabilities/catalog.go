...
var catalog = []Capability{
	...
	parity("sso.auth.mfa.set", "Set MFA mode", "POST", "/v1/auth/mfa", "sso_auth_mfa_set", "", []string{ScopeWrite}, true, true),
	parity("sso.user.totp.enroll", "Enroll TOTP overlay", "POST", "/v1/users/{id}/totp:enroll", "sso_user_totp_enroll", "", []string{ScopeWrite}, true, false),
	parity("sso.user.totp.clear", "Clear TOTP overlay", "POST", "/v1/users/{id}/totp:clear", "sso_user_totp_clear", "", []string{ScopeWrite}, true, true),
