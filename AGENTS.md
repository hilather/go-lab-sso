...
This repository has **implemented through VEN-003** (default ship + VEN-001 + OVR-001 + SAML-001 + UI-001 + IMP-001 + remaining clothes + WS-Fed + Duo/SiteMinder/Shibboleth). INT-001 is documented here and implemented last in `hilather/mcp-integration-lab`. SCIM-001 is design-only (`docs/23-scim-outbound.md`). Do not vendor Origin/Cursor agent-skills. Makefile targets that are not implemented must `false`, not no-op.
